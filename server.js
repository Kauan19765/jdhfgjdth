...
// Handler robusto via raw TCP (substituir o handler /stream/; existente)
app.get('/stream/;', (req, res) => {
  const UP_HOST = 'sonicpanel.oficialserver.com';
  const UP_PORT = 8342;
  const UP_PATH = '/;'; // mantém o ';'

  console.log('[raw-proxy] iniciando conexão TCP ->', `${UP_HOST}:${UP_PORT}${UP_PATH}`);

  const socket = net.connect({ host: UP_HOST, port: UP_PORT }, () => {
    // envia GET em HTTP/1.0 (muitos shoutcast preferem HTTP/1.0 / ICY)
    const request = [
      `GET ${UP_PATH} HTTP/1.0`,
      `Host: ${UP_HOST}`,
      `User-Agent: Node-ICY-Proxy/1.0`,
      `Icy-MetaData: 1`,
      `Connection: close`,
      `Accept: */*`,
      `\r\n`
  ...
  function closeSocketAndRes(code = 502, message = 'Upstream error') {
  ...
  socket.on('data', chunk => {
    if (headersParsed) {
      // cabeçalhos já foram enviados — encaminha chunk diretamente
      res.write(chunk);
      return;
    }

    // acumula até o final dos headers (\r\n\r\n)
    headerBuffer = Buffer.concat([headerBuffer, chunk]);
    const str = headerBuffer.toString('binary'); // binary pra não quebrar bytes de áudio acidentalmente
    const idx = str.indexOf('\r\n\r\n');

    if (idx !== -1) {
      // separa cabeçalhos do primeiro pedaço de body
      const headerPart = str.slice(0, idx);
      const remaining = headerBuffer.slice(idx + 4);

      // parse dos headers (primeira linha pode ser 'ICY 200 OK' ou 'HTTP/1.0 200 OK')
      const lines = headerPart.split(/\r\n/);
      const statusLine = lines.shift() || '';
      let statusCode = 200;
      if (/^\s*ICY\s+200/i.test(statusLine) || /^\s*HTTP\/\d+\.\d+\s+200/.test(statusLine)) {
        statusCode = 200;
      } else {
        // tenta extrair código se houver
        const m = statusLine.match(/\s(\d{3})/);
        if (m) statusCode = parseInt(m[1], 10);
      }

      // constrói objeto de headers
      const upstreamHeaders = {};
      lines.forEach(line => {
        const idx2 = line.indexOf(':');
        if (idx2 > 0) {
          const k = line.slice(0, idx2).trim().toLowerCase();
          const v = line.slice(idx2 + 1).trim();
          upstreamHeaders[k] = v;
        }
      });

      // define headers no response para o cliente
      const ct = upstreamHeaders['content-type'] || 'audio/mpeg';
      try {
        res.setHeader('Content-Type', ct);
        // repassa cabeçalhos icy se existirem
        ['icy-metaint','icy-br','icy-name','icy-description','icy-genre'].forEach(h => {
          if (upstreamHeaders[h]) res.setHeader(h, upstreamHeaders[h]);
        });
        // CORS + exposição
        ...
        console.warn('[raw-proxy] erro setHeader:', e && e.message);
      }

      headersParsed = true;

      // envia qualquer restante do body que veio junto com os headers
      if (remaining && remaining.length) {
        res.write(remaining);
      }

      // a partir daqui, os próximos 'data' serão pipeados direto (já coberto no topo)
    }
  });

  socket.on('end', () => {
    console.log('[raw-proxy] upstream ended');
    try { res.end(); } catch(e){}
  });

  socket.on('timeout', () => {
    console.error('[raw-proxy] socket timeout');
    closeSocketAndRes(504, 'Upstream timeout');
  });

  socket.on('error', (err) => {
    console.error('[raw-proxy] socket error:', err && (err.code || err.message));
    closeSocketAndRes(502, 'Erro ao conectar upstream: ' + (err && err.message));
  });

  req.on('close', () => {
    // cliente desconectou — fechar socket upstream
    try { socket.destroy(); } catch(e){}
    console.log('[raw-proxy] cliente desconectou');
  });
});
